...
export class CreateOrder {
  ...
  async execute(input: { codigo: string; descricao?: string }, correlationId: string) {
    return this.uow.run(async (tx) => {
      const now = this.clock.now();
      const order = Order.create(this.ids.next(), input.codigo, input.descricao, now);
      await tx.orders.save(order);
      await tx.outbox.append(envelopes(order.pullEvents(), this.ids, now, correlationId, null));
